...
export function Samsung({
  ...
  const videoTexture = useVideoTexture("/3d/samsungAI.mp4", {
    start: false,
    loop: false,
  });
  ...
  const screenMeshRef = usePrivacyDisplay(materials["Display_Activearea.001"]);
  ...
  useControls("Transition", {
    ...
    color1: {
      value: "#6e5da4",
      onChange: (v) => mat()?.uniforms.uColor1.value.set(v),
    },
    color2: {
      value: "#1a1a2e",
      onChange: (v) => mat()?.uniforms.uColor2.value.set(v),
    },
    DotEdges: folder({
      dotDensity: {
        value: 324,
      ...
      noisePositionX: {
        ...
        max: 1,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uNoisePosition.value.x = v;
        },
      },
      noisePositionY: {
        value: 0.08,
        ...
        max: 1,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uNoisePosition.value.y = v;
        },
      },
      noisePositionZ: {
        value: -0.04,
        ...
        max: 1,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uNoisePosition.value.z = v;
        },
      },
      edgeThickness: {
        value: 0.03,
        ...
        step: 0.001,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeThickness.value = v;
        },
      },
      maskMinY: {
        value: -1.0,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uMinMaxGradientMaskY.value.x = v;
        },
      },
      maskMaxY: {
        value: 1.0,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uMinMaxGradientMaskY.value.y = v;
        },
      },
    }),
    EdgeColor: folder({
      ...
      edgeColorStop1: {
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeColorStop1.value = v;
        },
      },
      edgeColorStop2: {
        value: 0.32,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeColorStop2.value = v;
        },
      },
      edgeColorStop3: {
        value: 0.44,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeColorStop3.value = v;
        },
      },
      edgeColorStop4: {
        value: 0.69,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeColorStop4.value = v;
        },
      },
      edgeColorWidth: {
        value: 0.06,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uEdgeColorWidth.value = v;
        },
      },
    }),
    OverlayDot: folder({
      ...
      overlayDotSize: {
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uOverlayDotSize.value = v;
        },
      },
      overlayDotDensity: {
        value: 385,
        ...
        step: 1,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uOverlayDotDensity.value = v;
        },
      },
      overlayMaskWidth: {
        value: 1.34,
        ...
        step: 0.01,
        onChange: (v) => {
          if (mat()?.uniforms) mat().uniforms.uOverlayMaskWidth.value = v;
        },
      },
    }),
  });
  ...
  useEffect(() => {
    const scheme = COLOR_SCHEMES[currentPhoneColor] ?? COLOR_SCHEMES["#6e5da4"];
    materials["Rearcase.001"].color.set(scheme.frameColor);
    setFrameColorObject(materials["Rearcase.001"].color);

    // Adjust emissive intensity of the screen and assign video emissiveMap
  ...
  }, [materials, setFrameColorObject, videoTexture]); // eslint-disable-line react-hooks/exhaustive-deps

  // Listen for video end to reset isVideoPlaying state
  useEffect(() => {
    const video = videoTexture.image as HTMLVideoElement;
    const handleEnded = () => setIsVideoPlaying(false);
    video.addEventListener("ended", handleEnded);
    return () => video.removeEventListener("ended", handleEnded);
  }, [videoTexture, setIsVideoPlaying]);

  // Play the video from the start when triggered
  useEffect(() => {
    if (!isVideoPlaying) return;
    const video = videoTexture.image as HTMLVideoElement;
    video.currentTime = 0;
    video.play();
  }, [isVideoPlaying, videoTexture]);
