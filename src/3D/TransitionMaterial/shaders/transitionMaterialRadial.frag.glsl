...
// 3D Hash (MOVED TO TOP SO VORONOI CAN SEE IT)
vec3 hash33(vec3 p) {
...
// Fractal Brownian Motion (Blender's default FBM noise loop)
float fbm(vec3 p, int octaves, float roughness) {
    ...
    for(int i = 0; i < 8; i++) {
        ...
        f += snoise(p) * amp;
        maxAmp += amp;
        p *= 2.0; // Lacunarity is 2.0 in the JSON
        amp *= roughness;
    }
    return f / maxAmp;
}

// Worley/Voronoi Distance (F1)
float voronoi(vec3 x) {
    return length(fract(x) - 0.5);
}

// ==========================================
// 3. BLEND MODES (Strictly matched to Blender logic)
// ==========================================
...
float blendSoftLight(float a, float b, float fac) {
    return mix(a, b < 0.5 ? 2.0 * a * b + a * a * (1.0 - 2.0 * b) : 2.0 * a * (1.0 - b) + sqrt(a) * (2.0 * b - 1.0), fac);
}
float blendMultiply(float a, float b, float fac) {
    return mix(a, a * b, fac);
}
float blendScreen(float a, float b, float fac) {
    return mix(a, 1.0 - (1.0 - a) * (1.0 - b), fac);
}

// Blender's "Color" Blend Mode replicates HSV color swap
float getLum(vec3 c) {
...
// ==========================================
// 4. COLOR RAMPS (Exact mappings from JSON)
// ==========================================
...
float ramp004(float t) {
    return map(t, 0.0, 0.344, 0.0, 1.0);
} // Matched JSON bounds
...
TransitionResult transition_fx(vec3 pos, vec2 uv) {
    ...
    // Generate accurate base noises from JSON attributes
    // Normalize Blender output bounds -1..1 to 0..1
    float noise003 = fbm(pos * 17.4 + vec3(0.0, 0.0, 2.3), 11, 0.6) * 0.5 + 0.5; // Detail 11
    float musgrave003 = fbm(pos * 8.99, 2, 0.03) * 0.5 + 0.5; // Detail 2.42 -> 2 octaves
    ...
    // ------------------------------------------------------------------
    // SPHERICAL DISTANCE MASK (Derived from Map Range.002 + Vector Math LENGTH)
    // ------------------------------------------------------------------
    float dist = length(pos); 

    // Blender equivalent: smoothstep interpolating uProgress to uProgress+1.51 towards 1.0 to 0.0
    float gradientMask = 1.0 - smoothstep(uProgress, uProgress + 1.51, dist);

    // Mix.008 & Mix.009 -> Base Mask (Using JSON accurate factors: 0.15 and variable NoiseStrength defaults to 0.05)
    float mix008 = blendLinearLight(noise003, musgrave003, 0.15);
    float baseMask = blendColorDodge(gradientMask, mix008, uNoiseStrength);
    ...
    vec3 mix005 = mix(mix013, vec3(blendSoftLight(mix013.r, mix004, 1.0)), 1.0);

    // Mix.012 (Screen Blend matches JSON)
    vec3 mix012 = blendScreen(mix005.r, mix014.r, 1.0) * max(mix005, mix014); // Ensures vector retains highest luminance per channel mathematically

    TransitionResult res;
    // Mix.010: Determines final Factor for Mix Shader.003
...
void main() {
    ...
    vec3 matA_Color = vec3(0.0); // Adjust as needed
    float matA_Roughness = 0.4;
    float matA_Metalness = 1.0;

    // Material B (Principled BSDF.002 Default Base)
    vec3 matB_Color = vec3(1.0, 0.09, 0.09); // Adjust as needed
