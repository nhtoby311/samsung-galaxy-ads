...
void main() {
    ...
    float distanceFromTransition = abs(remappedValue - transitionCenter);
    // Gradient mask with falloff
    float gradientMask = smoothstep(transitionCenter - uEdgeThickness, transitionCenter + uEdgeThickness, remappedValue);
    ...
    vec3 color = mix(uColor1, uColor2, dotPattern);
