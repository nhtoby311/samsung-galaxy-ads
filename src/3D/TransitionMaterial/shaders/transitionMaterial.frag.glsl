...
TransitionResult transition_fx(vec3 pos, vec2 uv) {
    ...
    // TOP-TO-BOTTOM LINEAR MASK
    float bandWidth = 1.51;
    float startY = uMinMaxGradientMaskY.y;
    float endY = uMinMaxGradientMaskY.x - bandWidth;
    float currentY = mix(startY, endY, uProgress);
    float gradientMask = smoothstep(currentY, currentY + bandWidth, pos.y);

    // Mix.008 & Mix.009 -> Base Mask (Both have clamp_result: true in JSON!)
