...
export default function Background() {
  ...
  const { colorsCount, distortion, swirl, grainMixer, grainOverlay } =
    useControls("Background", {
      ...
      Colors: folder({
        bgColor1: {
          value: "#4b38ab",
          onChange: (v) => useAppStore.getState().bgColorObjects[0].set(v),
        },
        bgColor2: {
          value: "#5e77c1",
          onChange: (v) => useAppStore.getState().bgColorObjects[1].set(v),
        },
        bgColor3: {
          value: "#8077e5",
          onChange: (v) => useAppStore.getState().bgColorObjects[2].set(v),
        },
        bgColor4: {
          value: "#b9beff",
          onChange: (v) => useAppStore.getState().bgColorObjects[3].set(v),
        },
        bgColor5: {
          value: "#dc93ff",
          onChange: (v) => useAppStore.getState().bgColorObjects[4].set(v),
        },
        bgColor6: {
          value: "#e1beff",
          onChange: (v) => useAppStore.getState().bgColorObjects[5].set(v),
        },
      }),
    });
  ...
  const uniforms = useMemo(
    () => ({
      u_time: { value: 0 },
      u_colors: {
        value: Array.from({ length: 10 }, () => new THREE.Vector4(0, 0, 0, 0)),
      },
